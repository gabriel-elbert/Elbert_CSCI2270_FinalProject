...
Station* Railroad::getLocation()
{
	return passenger.location;
}

Station* Railroad::findStation(string id)
{
	if (id.length() != 3)
	{
...
void Railroad::buyTicket(Station* dest, string un)
{
...
int Railroad::calcTotalDistance(Station *userLocation, Station *destination)
{
	int dist;
...
double Railroad::calcTravelTime()
{
	return (passenger.travelDistance / 38.5);
}

double Railroad::calcTicketPrice()
{
	char choice;
	double price;
	double business;
